...
export async function POST(req: Request) {
  try {
    ...
    let file_data = await fs.readFile(
      process.cwd() + "/src/app/dictionaries/ijp.json",
      "utf8"
    );
    let data = JSON.parse(file_data);
    data.our_products[payload.get("lang")].title = payload.get("title");
    data.our_products[payload.get("lang")].description =
    ...
    for (let index = 0; index < products.length; index++) {
      if (payload.get(`file_${index + 1}`)) {
        ...
        await fs.writeFile(
          path.join(
            process.cwd(),
            `public/images/our-products/products-ijp-${index + 1}.${
              file.type.split("/")[1]
            }`
          ),
          buffer
        );
        products[index].image_url = `/api/media/our-products/products-ijp-${
          index + 1
        }.${file.type.split("/")[1]}`;
      }
    }
    ...
    for (let index = 0; index < productsOthers.length; index++) {
      if (payload.get(`file_other_${index + 1}`)) {
        ...
        await fs.writeFile(
          path.join(
            process.cwd(),
            `public/images/our-products/products-ijp-others-${index + 1}.${
              file.type.split("/")[1]
            }`
          ),
          buffer
        ...
        productsOthers[
          index
        ].image_url = `/api/media/our-products/products-ijp-others-${
          index + 1
        }.${file.type.split("/")[1]}`;
      }
    }
