...
export async function POST(req: Request) {
    try {
        ...
        if (payload.get("file_1")) {
          ...
          await fs.writeFile(`./public/images/ijp/profile-1.${file.type.split("/")[1]}`, buffer);

          data.profile.image_1_url = `/images/ijp/profile-1.${file.type.split("/")[1]}`
        }
        ...
        if (payload.get("file_2")) {
          ...
          const arrayBuffer = await file.arrayBuffer();
          const buffer = new Uint8Array(arrayBuffer);
          await fs.writeFile(`./public/images/ijp/profile-2.${file.type.split("/")[1]}`, buffer);

          data.profile.image_2_url = `/images/ijp/profile-2.${file.type.split("/")[1]}`
        }
        ...
        if (payload.get("file_3")) {
          ...
          const arrayBuffer = await file.arrayBuffer();
          const buffer = new Uint8Array(arrayBuffer);
          await fs.writeFile(`./public/images/ijp/profile-3.${file.type.split("/")[1]}`, buffer);

          data.profile.image_3_url = `/images/ijp/profile-3.${file.type.split("/")[1]}`
        }
